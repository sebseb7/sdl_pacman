...
#include "pacman_icon.c"

	// these masks are needed to tell SDL_CreateRGBSurface(From)
	// to assume the data it gets is byte-wise RGB(A) data
	Uint32 rmask, gmask, bmask, amask;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
	int shift = (pacman_icon.bytes_per_pixel == 3) ? 8 : 0;
	...
	gmask = 0x0000ff00;
	bmask = 0x00ff0000;
	amask = (pacman_icon.bytes_per_pixel == 3) ? 0 : 0xff000000;
#endif

	SDL_Surface* icon = SDL_CreateRGBSurfaceFrom((void*)pacman_icon.pixel_data, pacman_icon.width,
			pacman_icon.height, pacman_icon.bytes_per_pixel*8, pacman_icon.bytes_per_pixel*pacman_icon.width,
